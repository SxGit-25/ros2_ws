...
namespace AnoProtocol {
    ...
    static const uint8_t ID_IMU_RAW     = 0x01; // 惯性传感器数据 [cite: 1361]
    static const uint8_t ID_EULER       = 0x03; // 欧拉角姿态 [cite: 1375]
    static const uint8_t ID_QUAT        = 0x04; // 四元数姿态 [cite: 1381]
    static const uint8_t ID_VELOCITY    = 0x07; // 飞行速度数据 [cite: 1414]
    static const uint8_t ID_OPTICAL_FLOW= 0x51; // 光流数据 [cite: 1537]
    static const uint8_t ID_BATTERY     = 0x0D; // 电压电流 [cite: 1445]

    // --- 发送类 (TX, To Flight Controller) ---
    static const uint8_t ID_EXT_VEL     = 0x33; // 通用速度传感器 (用于EKF闭环) [cite: 1507]
    static const uint8_t ID_CONTROL     = 0x41; // 实时控制帧 (用于Nav2控制) [cite: 1526]

    // ==========================================
    // 2. 数据载荷结构体 (Payload)
    // 注意：协议采用小端模式 (Little Endian) [cite: 1310]
    // ==========================================

    #pragma pack(push, 1) // 强制1字节对齐

    /**
     * @brief ID: 0x01 惯性传感器数据 [cite: 1364]
     * 长度: 13 bytes
     */
    ...
    /**
     * @brief ID: 0x04 四元数格式姿态 [cite: 1384]
     * 长度: 9 bytes
     * 说明: 数据传输时扩大了 10000 倍 [cite: 1385]
     */
    ...
    /**
     * @brief ID: 0x51 匿名光流数据 (Mode 1: 融合光流) [cite: 1545]
     * 长度: 5 bytes (最小长度)
    ...
    /**
     * @brief ID: 0x33 通用速度型传感器数据 (TX) [cite: 1510]
     * 长度: 6 bytes
     * 说明: 用于将 EKF 算出的速度传回飞控，单位 cm/s
     * 坐标系: 捷联载体坐标系 (X机头，Y机左) [cite: 1512]
     */
    ...
    /**
     * @brief ID: 0x41 实时控制帧 (TX) [cite: 1529]
     * 长度: 14 bytes
     * 说明: 用于 Nav2 控制飞控飞行
     */
    struct Frame0x41_Control {
        ...
        int16_t ctrl_yaw_dps; // 偏航角速度 deg/s [cite: 1532]
        int16_t ctrl_spd_x;   // 期望速度 X cm/s [cite: 1533]
